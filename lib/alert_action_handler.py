...
def run_trigger_actions(global_config_data, system_config_data, trigger_config, alert_data, call_data):
    ...
    if system_config_data.get("email_enabled", False):
        ...
        subject, body = generate_trigger_alert_email(global_config_data, system_config_data, trigger_config, alert_data, call_data)

        if not subject or not body:
            module_logger.warning(f"Skipping Sending Email for Trigger {trigger_config.get('trigger_name')} Unable to get email body or subject.")
        else:
            email_list = []
            ...
            if len(email_list) >= 1:
                ...
                module_logger.warning(f"Skipping Sending Email for Trigger {system_config_data.get('system_name')} No alert emails set")

    # Send Trigger Alert Pushover
    if system_config_data.get("pushover_enabled", False):
        PushoverSender(global_config_data, system_config_data, trigger_config).send_text_alert_push(alert_data, call_data)
...
def run_global_actions(global_config_data, system_config_data, alert_data, call_data):
    ...
    if system_config_data.get("email_enabled", False):
        ...
        if not subject or not body:
            module_logger.warning(f"Skipping Sending Email for System {system_config_data.get('system_name')} Unable to get email body or subject.")
        else:
            email_list = []
            ...
            if len(email_list) >= 1:
                ...
                module_logger.warning(f"Skipping Sending <<Email>> for <<System>> {system_config_data.get('system_name')} No alert emails set")
